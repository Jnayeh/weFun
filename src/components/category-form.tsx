"use client";
import { useProgressStore, useCategoryStore } from "~/store/category-form";
    ...
    <>
      <div className="flex h-10v items-center justify-end bg-white px-5 dark:bg-slate-600">
        ...
        <div className="min-h-10v flex scale-75 flex-row items-end justify-between gap-1 py-2 sm:scale-100 sm:items-center">
          ...
          >
            <div
              className={`add_category_step-number ${
      ...
      <h1 className="w-[35%] min-w-[360px] self-center p-4 pt-12 text-center text-2xl font-bold text-secondary dark:text-primary">
        {progress == 3 ? "Add category" : "Add A category "}
      </h1>
    </>
  );
}
...
function ActivityButtons() {
  ...
  const addAct = api.category.create.useMutation({
    async onSuccess() {
      // refetches activities after a post is added
      nextRevalidate("getCategories");
    ...
    <div className=" flex w-[35%] min-w-[360px] justify-center gap-4 self-center p-2">
      ...
      {progress < 3 ? (
        ...
        <button
          ...
          onClick={(e) => {
            e.preventDefault();
            const form = useCategoryStore.getState();
...
//     }
//   );
//   const catId = useCategoryStore((state) => state.categoryId);
...
// }
 ...
 function AddCategoryForm() {
  ...
  const setLabel = useCategoryStore((state) => state.setLabel);
  const setDetails = useCategoryStore((state) => state.setDetails);
  const setCover = useCategoryStore(
  ...
  return (
    <>
      ...
      <textarea
        ...
        placeholder="Details of the category"
